...
#define ERR_FILE_FROM "Error: Can't read from file %s\n"
#define ERR_FILE_TO "Error: Can't read from file %s\n"
/**
...
int main(int argc, char *argv[])
{
	...
	if (fileNum == -1)
		dprintf(STDERR_FILENO, ERR_FILE_FROM, argv[1]), exit(98);
	fileNum2 = open(argv[2], O_CREAT | O_WRONLY | O_TRUNC, 0664);

	if (fileNum2 == -1)
	{
		dprintf(STDERR_FILENO, ERR_FILE_TO, argv[2]);
		exit(99);
	}

	while ((b = read(fileNum, buffer, BUFF_SIZE)) > 0)
		if (write(fileNum2, buffer, b) != b)
			dprintf(STDERR_FILENO, ERR_FILE_TO, argv[2]), exit(99);
	if (b == -1)
		dprintf(STDERR_FILENO, ERR_FILE_FROM, argv[1]), exit(98);
